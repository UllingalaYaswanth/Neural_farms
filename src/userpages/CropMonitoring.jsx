...
//       const stageValues = [
//         data.germination,
//         data.vegetative_growth,
//         data.flowering,
//         data.maturity,
//         data.harvesting,
//       ];
//       setLiveStages(stageValues);
//     })
//     .catch((error) => {
//       console.error("Error loading crop stage data", error);
//     });
// }, []);
...
//   datasets: [{
//     label: "Growth Stages (%)",
//     data: liveStages || [0, 0, 0, 0, 0],  // fallback while loading
//     borderColor: "#4caf50",
//     backgroundColor: "rgba(76, 175, 80, 0.2)",
//     fill: true,
//     tension: 0.1,
...
//     ] },
//  {
//     icon: <FaLeaf className="mr-2 text-3xl text-green-500" />,
//     title: "Growth Stage",
//     value: liveStages
//       ? `Current Stage: ${liveStages[3]}% of Maturation`
//       : "Loading...",
//     description: "Stage progress of the selected crop",
//     points: liveStages
//       ? [
//           `Germination: ${liveStages[0]}% complete.`,
//           `Vegetative Growth: ${liveStages[1]}% complete.`,
//           `Flowering: ${liveStages[2]}% complete.`,
//           `Maturation: ${liveStages[3]}% complete.`,
//           `Harvesting: ${liveStages[4]}% complete.`,
//         ]
//       : [],
//   },
...
import { Line, Bar } from "react-chartjs-2";
import { Chart as ChartJS, CategoryScale, LinearScale, Title, Tooltip, Legend, BarElement, LineElement } from "chart.js";
import { FaWater, FaExclamationCircle, FaSprayCan, FaLeaf, FaVirus, FaSync } from "react-icons/fa";
...
const CropMonitoring = () => {
  ...
  const refreshData = async () => {
    ...
    try {
      ...
      console.error("Error loading crop stage data", err);
      setError("Failed to load growth stage data. Please try again.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  useEffect(() => {
    ...
    // Refresh every hour (adjust as needed)
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-6">
        {[
          { 
            icon: <FaLeaf className="mr-2 text-3xl text-green-500" />, 
            title: "Crop Health", 
            value: `${currentCropData.cropHealth}%`, 
            description: "Health status of the selected crop", 
            points: [
              "Soil health: pH level at 6.5 (optimal range: 6.0–7.0).",
              "Water levels: Irrigation at 500 liters/day (recommended: 450–550 liters/day).",
              "Pest control: Only 5% of leaves show pest damage (threshold: <10%).",
          ...
          },
          {
            ...
            value: liveStages
              ? `Current Stage: ${liveStages[3]}% of Maturation`
              : isLoading ? "Loading..." : "Data not available",
            description: "Stage progress of the selected crop",
            points: liveStages
              ? [
                  `Germination: ${liveStages[0]}% complete.`,
          ...
          { 
            icon: <FaWater className="mr-2 text-3xl text-blue-500" />, 
            title: "Water Level", 
            value: currentCropData.waterLevel, 
            description: "Current water level status of the crop",
            points: [
              "Soil moisture: 70% (optimal range: 65–75%).",
              "Weather conditions: 50mm rainfall this week (average: 40–60mm).",
              "Irrigation system: Delivering 500 liters/day (crop requirement: 450–550 liters/day).",
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-6 mb-6">
        <div className="bg-white shadow-lg rounded-lg p-6">
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-xl font-semibold">Crop Growth Stages</h2>
            {isLoading && <span className="text-sm text-gray-500">Updating data...</span>}
          </div>
          {error ? (
          ...
          {lastRefresh && (
            <p className="text-xs text-gray-500 text-right mt-2">
              Data from: {new Date(lastRefresh).toLocaleTimeString()}
            </p>
          )}
        </div>
        <div className="bg-white shadow-lg rounded-lg p-6">
